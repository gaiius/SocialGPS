...
public class contact_sync {
	...
	{		
		...
		} catch(Exception e){	Log.e("Exception main ",e.toString()); }
		finally { d.close(); }
	}
		...
		public List<user_detail_dao> get_sync_contact(ContentResolver x)
	{
     ...
     reuse=add_display_name(reuse, ans);
  	 return reuse;
	}catch(Exception e){	Log.e("Exception 2 ",e.toString()); return null;	}
	}
	...
	public List add_display_name(List<user_detail_dao> without,List<user_detail_dao> with)
	{
		try	{
			...
			{ System.out.print("i");
				...
				{ System.out.print("k");
					if(without.get(i).get_user_id().equals(with.get(j).get_user_id()))
					{
						without.get(i).set_display_name(with.get(j).get_display_name());
					}
				}
			}
			return without;  //now display_name added 
		}catch(Exception e){	Log.e("Exception 4 ",e.toString()); return null;	}
	...
	public List readContacts(ContentResolver x){
		 try	{
		  ...
		  catch(Exception e){	Log.e("Exception 1 ",e.toString()); return null;	}
		  //finally	{ cur.close();	}
	  }
